(function () {
    ...
    function update(info) {
        Array.from(document.querySelectorAll(".container,.page")).map((ele, i, arr) => {
            var width = parseInt(ele.parentNode.style.width) || innerWidth;
            var height = parseInt(ele.parentNode.style.height) || innerHeight;
            layout("line", ele, width, height, true);
        });
        Array.from(document.querySelectorAll("[layer]")).map(ele => ele.style.zIndex = ele.attributes.layer.value);
        Array.from(document.querySelectorAll("[color],[shade]")).map(ele => {

            var c = ele.attributes.color != null ?
                ele.attributes.color.value.toLowerCase() :
                "grey";

            var type = ele.attributes.shade != null ?
                ele.attributes.shade.value :
                "500";
            if (c == "none") ele.style.color = ele.style.backgroundColor = "";
            else {
                var code = colors[c][type];
                if (["p", "h1", "h2", "h3"]
                    .includes(ele.nodeName.toLowerCase()))
                    ele.style.color = code;
                else
    ...
    function layout(type, ele, width, height, master) {
        ...
        var layoutguide = ratioCalculate({ row: width, line: height }[type],
            eles.map(l => {
                if (l.attributes.layout == undefined) return "*1";
                return l.attributes.layout.value;
            }));
        eles.map((ele, i) => {
    ...
    var targetx = 0;
    var targety = 0;
    function scrollToAnimation(ele, x, y, i) {
        targetx = x;
        targety = y;
        if (i <= 0) {
            ele.scrollLeft = targetx;
            ele.scrollTop = targety;
        }
        else {

            ele.scrollLeft += (targetx - ele.scrollLeft) * 0.3;
            ele.scrollTop += (targety - ele.scrollTop) * 0.3;

            setTimeout(function () {
                scrollToAnimation(ele, targetx, targety, i - 1);
            }, 14);
        }
    }

    function scrollToPage(element, noAnim) {
        var html = pages[0].getBoundingClientRect();
        var targ = element.getBoundingClientRect();

        var y = targ.top - html.top;
        var x = targ.left - html.left;
