(function () {
    ...
    function layoutCalc(r1, o1, r2, o2, s) {
        var o = o1 - s * r1 - o2 + s * r2;
        var r = r1 - r2 + 1;
        var tr = r1 / (r1 - r2);
        var t = s * r2 - o2 - o * tr;
    ...
    function layout(ele) {
        ...
        var layoutGuide = childs.map(ele => {
            if (ele.attributes.layout != null) return ele.attributes.layout.value;
            return "*1";
        }).map(guide => {
            if (guide[0] == '*') return { type: "ratio", value: +(guide.substr(1)) }
            return { type: "real", value: +(guide) }
        });
        var sumRatio = 0;
        var sumReal = 0;
        var sum = layoutGuide.filter(g => g.type == "real").reduce((a, b) => a + b.value, 0);
        var sumr = layoutGuide.filter(g => g.type == "ratio").reduce((a, b) => a + b.value, 0);

        childs.map((ele, i) => {
            var newRatio = sumRatio;
            var newReal = sumReal;
            if (layoutGuide[i].type == "ratio") newRatio += layoutGuide[i].value / sumr;
            if (layoutGuide[i].type == "real") newReal += layoutGuide[i].value;
            var data = layoutCalc(sumRatio, sumReal, newRatio, newReal, sum);
            sumRatio = newRatio;
            sumReal = newReal;

            ele.style.position = "absolute";
            ele.style.left = data.o + "px";
            ele.style.right = data.r * 100 + "%";
            ele.style.transform = `translate(${data.t}px, 0) translate(${data.tr * 100}%, 0)`;
        });
    }

    function update(info) {
        [...document.querySelectorAll('.row')].map(ele => {
            layout(ele);
